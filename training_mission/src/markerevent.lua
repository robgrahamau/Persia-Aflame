...
---@param _tankers boolean - do we handle tankers in markers
---@return table nil
function HEVENT:New(_markers,_lqm,_death,_tankertime,_tankercooldown, _tankers)
...
function HEVENT:OnEventMarkRemoved(EventData)
  if EventData.text~=nil and EventData.text:lower():find("-") then
    ...
    elseif mainkey == ("-tanker") then
      if self.TANKERS == true then
        self:handleTankerRequest(text2,coord,_coalition,_playername,_group) -- Confirmed
      else
        self:Msg("Unable to at this time, tanker commands are disabled",_group,nil,10)
      end
    ...
    elseif mainkey == ("-ctldfob") then -- confirmed.
      ...
      if ADMIN == true then
        if CTLD ~= nil then
          ...
          self:Msg("Unable, CTLD is not enabled on this server",_group,nil,10)
        end
      else
