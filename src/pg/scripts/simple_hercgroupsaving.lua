...
-- Usage of this script should credit the following contributors:
 ...
 local SaveScheduleUnits=300 --how many seconds between each check of all the Units.
 HercGroups = SET_GROUP:New():FilterCategories("ground"):FilterPrefixes({"HD","HD_CARGO"}):FilterActive(true):FilterStart()
 local savefilename = "pg_hercgroups.lua"
...
if resetall == 0 then
  if file_exists(savefile) then --Script has been run before, so we need to load the save
    ...
    HercGroups:ForEachGroup(function (grp)
      destroycounter = destroycounter + 1
      grp:Destroy()
    end)
    env.info("Destroyed a total of ".. destroycounter .." Groups")
	HypeMan.sendBotMessage("Herc Group Saving: Destroyed a total of ".. destroycounter .." Groups")
  dofile(savefile)
...
HypeMan.sendBotMessage("Herc Group Saving: spawned in a total of ".. spawncounter .." Groups")
--THE SAVING SCHEDULE
SCHEDULER:New( nil, function()
  HercGroups:ForEachGroupAlive(function (grp)
