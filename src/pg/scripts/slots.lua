-- Airbases

abbasslots = slothandler:New(AIRBASE.PersianGulf.Bandar_Abbas_Intl,1,"Abbas")
abbasslots:SetRedCTLD(90,1500,true,country.id.CJTF_RED)
abbasslots:SetBlueCTLD(90,2000,true,country.id.CJTF_BLUE)
abbasslots:setroutegroups(true,11000)
abbasslots:ActivateCTLD()
abbasslots:Start()

abudbslots = slothandler:New(AIRBASE.PersianGulf.Abu_Dhabi_International_Airport,2,"AbuDhabi")
abudbslots:SetRedCTLD(180,1400,true,country.id.CJTF_RED)
abudbslots:SetBlueCTLD(170,1700,true,country.id.CJTF_BLUE)
abudbslots:setroutegroups(true,15000)
abudbslots:ActivateCTLD()
abudbslots:Start()

abunuslots = slothandler:New(AIRBASE.PersianGulf.Sir_Abu_Nuayr,2,"AbuNu")
abunuslots:SetRedCTLD(320,500,true,country.id.CJTF_RED)
abunuslots:SetBlueCTLD(330,600,true,country.id.CJTF_BLUE)
abunuslots:setroutegroups(true,5000)
abunuslots:ActivateCTLD()
abunuslots:Start()

dhafraslots = slothandler:New(AIRBASE.PersianGulf.Al_Dhafra_AB,2,"Dhafra")
dhafraslots:SetBlueCTLD(090,3000,true,country.id.CJTF_BLUE)
dhafraslots:setroutegroups(true,15000)
dhafraslots:ActivateCTLD()
dhafraslots:Start()

ejaskslots = slothandler:New(AIRBASE.PersianGulf.Bandar_e_Jask_airfield,1,"E-Jask")
ejaskslots:SetRedCTLD(40,1400,true,country.id.CJTF_RED)
ejaskslots:SetBlueCTLD(50,1700,true,country.id.CJTF_BLUE)
ejaskslots:setroutegroups(true,15000)
ejaskslots:ActivateCTLD()
ejaskslots:Start()

fujairahslots = slothandler:New(AIRBASE.PersianGulf.Fujairah_Intl,2,"Fujairah")
fujairahslots:SetRedCTLD(180,500,true,country.id.CJTF_RED)
fujairahslots:SetBlueCTLD(200,700,true,country.id.CJTF_BLUE)
fujairahslots:setroutegroups(true,15000)
fujairahslots:ActivateCTLD()
fujairahslots:Start()

havslots = slothandler:New(AIRBASE.PersianGulf.Havadarya,1,"Havadarya")
havslots:SetRedCTLD(0,500,true,country.id.CJTF_RED)
havslots:SetBlueCTLD(10,700,true,country.id.CJTF_BLUE)
havslots:setroutegroups(true,10000)
havslots:ActivateCTLD()
havslots:Start()

jiroftslots = slothandler:New(AIRBASE.PersianGulf.Jiroft_Airport,1,"Jiroft")
jiroftslots:SetRedCTLD(40,500,true,country.id.CJTF_RED)
jiroftslots:SetBlueCTLD(40,800,true,country.id.CJTF_BLUE)
jiroftslots:setroutegroups(true,35000)
jiroftslots:ActivateCTLD()
jiroftslots:Start()


kermanslots = slothandler:New(AIRBASE.PersianGulf.Kerman_Airport,1,"Khasab")
kermanslots:SetRedCTLD(270,2000,true,country.id.CJTF_RED)
kermanslots:SetBlueCTLD(270,2500,true,country.id.CJTF_BLUE)
kermanslots:setroutegroups(true,45000)
kermanslots:ActivateCTLD()
kermanslots:Start()

khasabslots = slothandler:New(AIRBASE.PersianGulf.Khasab,2,"Khasab")
khasabslots:SetRedCTLD(240,600,true,country.id.CJTF_RED)
khasabslots:SetBlueCTLD(240,800,true,country.id.CJTF_BLUE)
khasabslots:setroutegroups(true,15000)
khasabslots:ActivateCTLD()
khasabslots:Start()

kishslots = slothandler:New(AIRBASE.PersianGulf.Kish_International_Airport,1,"Kish")
kishslots:SetRedCTLD(220,1400,true,country.id.CJTF_RED)
kishslots:SetBlueCTLD(240,1500,true,country.id.CJTF_BLUE)
kishslots:setroutegroups(true,15000)
kishslots:ActivateCTLD()
kishslots:Start()

larslots = slothandler:New(AIRBASE.PersianGulf.Lar_Airbase,1,"Lar")
larslots:SetRedCTLD(10,1000,true,country.id.CJTF_RED)
larslots:SetBlueCTLD(170,1000,true,country.id.CJTF_BLUE)
larslots:setroutegroups(true,25000)
larslots:ActivateCTLD()
larslots:Start()

lavanslots = slothandler:New(AIRBASE.PersianGulf.Lavan_Island_Airport,1,"Lavan")
lavanslots:SetRedCTLD(240,600,true,country.id.CJTF_RED)
lavanslots:SetBlueCTLD(240,800,true,country.id.CJTF_BLUE)
lavanslots:setroutegroups(true,15000)
lavanslots:ActivateCTLD()
lavanslots:Start()

liwaslots = slothandler:New(AIRBASE.PersianGulf.Liwa_Airbase,2,"Liwa")
liwaslots:SetRedCTLD(240,600,true,country.id.CJTF_RED)
liwaslots:SetBlueCTLD(240,800,true,country.id.CJTF_BLUE)
liwaslots:setroutegroups(true,15000)
liwaslots:ActivateCTLD()
liwaslots:Start()

lengehslots = slothandler:New(AIRBASE.PersianGulf.Bandar_Lengeh,1,"Lengeh")
lengehslots:SetRedCTLD(05,1000,true,country.id.CJTF_RED)
lengehslots:SetBlueCTLD(25,1000,true,country.id.CJTF_BLUE)
lengehslots:setroutegroups(true,25000)
lengehslots:ActivateCTLD()
lengehslots:Start()

-- Al-Minhad
minhadslots = slothandler:New(AIRBASE.PersianGulf.Al_Minhad_AB,2,"Minhad")
minhadslots:SetBlueCTLD(180,2000,true,country.id.CJTF_BLUE)
minhadslots:ActivateCTLD()
minhadslots:setroutegroups(true,5000)
minhadslots:Start()

musaslots = slothandler:New(AIRBASE.PersianGulf.Abu_Musa_Island_Airport,2,"Musa")
musaslots:SetRedCTLD(140,5000,true,country.id.CJTF_RED)
musaslots:SetBlueCTLD(150,7000,true,country.id.CJTF_BLUE)
musaslots:ActivateCTLD()
musaslots:setroutegroups(true,5000)
musaslots:Start()

qeshmslots = slothandler:New(AIRBASE.PersianGulf.Qeshm_Island,1,"Qeshm")
qeshmslots:SetRedCTLD(320,900,true,country.id.CJTF_RED)
qeshmslots:SetBlueCTLD(330,1100,true,country.id.CJTF_BLUE)
qeshmslots:setroutegroups(true,15000)
qeshmslots:ActivateCTLD()
qeshmslots:Start()

sirrislots = slothandler:New(AIRBASE.PersianGulf.Sirri_Island,1,"Sirri")
sirrislots:SetRedCTLD(290,2000,country.id.CJTF_RED)
sirrislots:SetBlueCTLD(290,2300,country.id.CJTF_BLUE)
sirrislots:ActivateCTLD()
sirrislots:setroutegroups(true,5000)
sirrislots:Start()

shirazslots = slothandler:New(AIRBASE.PersianGulf.Shiraz_International_Airport,1,"Shiraz")
shirazslots:SetRedCTLD(180,2000,country.id.CJTF_RED)
shirazslots:SetBlueCTLD(180,2300,country.id.CJTF_BLUE)
shirazslots:ActivateCTLD()
shirazslots:setroutegroups(true,50000)
shirazslots:Start()

tunbslots = slothandler:New(AIRBASE.PersianGulf.Tunb_Island_AFB,1,"Tunb")
tunbslots:SetRedCTLD(285,1200,country.id.CJTF_RED)
tunbslots:SetBlueCTLD(280,1400,country.id.CJTF_BLUE)
tunbslots:ActivateCTLD()
tunbslots:setroutegroups(true,5000)
tunbslots:Start()

-- Carriers

Teddyslot = carriersslothandler:New("Teddy","Teddy"):Start()
Washingtonslot = carriersslothandler:New("Washington","Washington"):Start()
Forrestalslot = carrierslothandler:New("Forrestal","Forrestal"):Start()
Kuznetsov = carrierslothandler:New("Kuznetsov","Kuznetsov"):Start()
Tarawaslot = carriersslothandler:New("Tarawa","Tarawa"):Start()
Saipanslot = carriersslothandler:New("Saipan","Saipan"):Start()
Nassauslot = carriersslothandler:New("Nassau","Nassau"):Start()
Peleliuslot = carriersslothandler:New("Peleliu","Peleliu"):Start()


-- Fobs
BN85 = fob:New("BN85","red_bn85","blue_bn85",2,0):Start()
BQ26 = fob:New("BQ26","red_BQ26","blue_BQ26",1,90):Start()
BQ51 = fob:New("BQ51","red_BQ51","blue_BQ51",1,0):Start()
BQ99 = fob:New("BQ99","red_bq99","blue_BQ99",1,0):Start()
BR31 = fob:New("BR31","red_BR31","blue_BR31",1,0):Start()
BS02 = fob:New("BS02","red_BS02","blue_BS02",1,0):Start()
CT75 = fob:New("CT75","red_CT75","blue_CT75",1,0):Start()
DP05 = fob:New("DP05","red_DP05","blue_DP05",2,180):Start()
DR35 = fob:New("DR35","red_DR35","blue_DR35",1,0):Start()
DU06 = fob:New("DU06","red_DU06","blue_DU06",1,180):Start()
EP36 = fob:New("EP36","red_EP36","blue_EP36"),1,180):Start()
EQ13 = fob:New("EQ13","red_EQ13","blue_EQ13"),1,180):Start()
ER00 = fob:New("ER00","red_ER00","blue_ER00"),1,270):Start()
FR47 = fob:New("FR47","red_FR47","blue_FR47"),1,180):Start()
YK65 = fob:New("YK65","red_YK65","blue_YK65"),1,180):Start()
YN50 = fob"New("YN50","red_YN50","blue_YN50"),1,180):Start()