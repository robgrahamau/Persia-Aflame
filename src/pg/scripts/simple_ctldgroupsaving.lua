...
-- Usage of this script should credit the following contributors:
 ...
 local SaveScheduleUnits=660 --how many seconds between each check of all the Units.
 CTLDAllGroups = SET_GROUP:New():FilterCategories("ground"):FilterPrefixes({"CTLD","ctld",}):FilterActive(true):FilterStart()
 local savefilename = "ctld_groups_pg.lua"
...
if resetall == 0 then
  if file_exists(savefile) then --Script has been run before, so we need to load the save
    ...
    CTLDAllGroups:ForEachGroup(function (grp)
      destroycounter = destroycounter + 1
      grp:Destroy()
    end)
    env.info("Destroyed a total of ".. destroycounter .." Groups")
	HypeMan.sendBotMessage("CTLD GroupSaving - Destroyed a total of ".. destroycounter .." Groups")
  dofile(savefile)
  tempTable={}
  Spawn={}
...
groupData = 
  ...
  env.info("No Saved File")
  SaveUnits={} 
end
else
  if resetall == 1 then
    env.info("PersistedStore.resetall was 1")
  end
  SaveUnits={}

end
HypeMan.sendBotMessage("CTLD GroupSaving - Spawned a total of ".. spawncounter .." Groups")
--THE SAVING SCHEDULE
SCHEDULER:New( nil, function()
  CTLDAllGroups:ForEachGroupAlive(function (grp)
