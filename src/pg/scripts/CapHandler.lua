...
---@return table|self
function CAPHANDLER:New(_name,_limit,_checkalive)
...
---handles debug msgs
---@param _msg any
function CAPHANDLER:RDebug(_msg)
...
function CAPHANDLER:SetCheckAlive(_value)
...
function CAPHANDLER:checkcount()
...
---@param _amount any
---@return integer
function CAPHANDLER:addcount(_amount)
...
---@param _amount any
---@return integer
function CAPHANDLER:remcount(_amount)
	local amount = _amount or 1
	self.Counter = self.Counter - amount
	if self.Counter < 0 then
		self.Counter = 0
...
---event dead
---@param EventData any
function CAPHANDLER:EDead(EventData)
...
---event land
---@param EventData any
function CAPHANDLER:ELand(EventData)
...
function CAPHANDLER:Respawn()
	...
	if self.Group ~= nil then
		...
		if self.Counter < self.Limit then
            ...
            -- checking if we own the airbase
            local coalself = self.Group:GetCoalition()
            local coalair = self.airbase:GetCoalition()
            if coalself ~= coalair then
                self.Group:Destroy()
                local _msg = string.format("ERROR We were unable to respawn %n as the group would not be spawning at a friendly airbase",self.Name)
		        rlog(_msg)
                return self
            end
...
---send a msg
---@param _msg any
function CAPHANDLER:SendMsg(_msg)
...
function CAPHANDLER:HeartBeat()
	...
	if self.Group:IsAlive() == true then
   	...
   	if self.checkalive == true then
	    if self.Group:IsAlive() ~= true then
            if self.airbase ~= nil then
                if self.coalition == self.airbase:GetCoalition() then
                    self:Respawn()
                end
            end
	    end
