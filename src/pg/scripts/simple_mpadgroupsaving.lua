...
-- Usage of this script should credit the following contributors:
 ...
 MpadGroups = SET_GROUP:New():FilterCategories("ground"):FilterPrefixes({"MANPAD","manpad"}):FilterActive(true):FilterStart()
 local savefilename = "pg_mpad_groups.lua"
...
if resetall == 0 then
  if file_exists(savefile) then --Script has been run before, so we need to load the save
    ...
    MpadGroups:ForEachGroup(function (grp)
      destroycounter = destroycounter + 1
      grp:Destroy()
    end)
    env.info("Destroyed a total of ".. destroycounter .." Groups")
	HypeMan.sendBotMessage("ManpadGroups - Destroyed a total of ".. destroycounter .." Groups")
  dofile(savefile)
  tempTable={}
  Spawn={}
...
HypeMan.sendBotMessage("ManPadGroups - Spawned a total of ".. spawncounter .." Groups")
--THE SAVING SCHEDULE
SCHEDULER:New( nil, function()
  MpadGroups:ForEachGroupAlive(function (grp)
